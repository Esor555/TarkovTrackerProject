...
namespace TTBusinesLogic.BusinesLogic
{
    internal class UserValidator
    {
        ...
        {
            string ErrorString = "";
            if (ValidateId(user.Id) == false)
            {
                ErrorString += $"UserID is wrong: {user.Id.ToString()}\n";
            }
            if (ValidateName(user.Name) == false)
            {
                ErrorString += $"UserName is wrong: {user.Name.ToString()} \n";
            }
            if (ValidateLevel(user.Level) == false)
            {
                ErrorString += $"UserLevel is wrong: {user.Level.ToString()}\n";
            }
            if (ValidateFaction(user.Faction) == false)
            {
                ErrorString += $"UserFaction is wrong: {user.Faction.ToString()}\n";
            }
            if (ErrorString == "")
            {
                return true;
